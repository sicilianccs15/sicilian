...
#include "v8.h"
//#include "core/dom/uglify.h"
//#include "core/dom/uglified_original.h"
//#include "core/dom/parse_esprima.h"
//#include "core/dom/ast_esprima_walk.h"
//#include "core/dom/ast_hash.h"
#include "core/dom/ast_random_hash.h"
String processJS(const char * source_code)
{
        ...
        //just get the source_code in there
        v8::Isolate* isolate = v8::Isolate::New();
        {
                v8::Isolate::Scope isolate_scope(isolate);
                v8::HandleScope handle_scope(isolate);
                v8::Handle<v8::ObjectTemplate> global = v8::ObjectTemplate::New();
	        global->Set(isolate, "code", v8::String::NewFromUtf8(isolate, source_code));
	        v8::Handle<v8::Context> context = v8::Context::New(isolate, NULL, global);
                
                v8::Context::Scope context_scope(context);
                v8::Handle<v8::Script> beautifyScript = v8::Script::Compile(v8::String::NewFromUtf8(isolate,uglify_code));
    	        beautifyScript->Run();

    	        v8::Handle<v8::Script> runnerScript = v8::Script::Compile(v8::String::NewFromUtf8(isolate,"rebuild(code)"));
    	        v8::Handle<v8::Value> result = runnerScript->Run();

    	        if (!result.IsEmpty() && !result->IsUndefined()) {
                        v8::String::Utf8Value str(result);

                        std::string s = *str;
                        WTF::String str1 = WTF::UTF8Encoding().decode(s.c_str(), s.size());
                        //printf("This is the result %s\n", *str);
                        //printf("The other one %s\n",s.c_str());
                        //take suggestions from chrome people
                        return str1;
                        //return s.c_str();
                        }
                return "";            
                //v8::Local<v8::String> source = v8::String::NewFromUtf8(isolate, "'Hello'");
...
void ScriptLoader::executeScript(const ScriptSourceCode& sourceCode, double* compilationFinishTime)
{
    ...
    if (m_isExternalScript) {
        ...
        int subresource_integrity_check = SubresourceIntegrity::CheckSubresourceIntegrity(*m_element, sourceCode.source(), sourceCode.resource()->url(), sourceCode.resource()->mimeType());
        
        if (subresource_integrity_check == 0){
            contextDocument->addConsoleMessage(ConsoleMessage::create(SecurityMessageSource, ErrorMessageLevel,"Hash Didnt match and no further structural computation"));
            return ;
        }
        
        //check semantic hash here
        else if(subresource_integrity_check == 2)
        {
          
          contextDocument->addConsoleMessage(ConsoleMessage::create(SecurityMessageSource, InfoMessageLevel,"Checking Semantic Integrity for  "+sourceCode.resource()->url().elidedString()));
          
          //see what chrome people haave said
          String processed_source = processJS(sourceCode.source().utf8().data());
          
          //contextDocument->addConsoleMessage(ConsoleMessage::create(SecurityMessageSource, InfoMessageLevel, "New source is "+processed_source));
                    
          if(!SubresourceIntegrity::CheckSemanticIntegrity(*m_element, processed_source)){
                contextDocument->addConsoleMessage(ConsoleMessage::create(SecurityMessageSource, ErrorMessageLevel," Semantic hash didnt match for resource found at "+sourceCode.resource()->url().elidedString()));
                          return;
            }
            //optional
           else
           {
           contextDocument->addConsoleMessage(ConsoleMessage::create(SecurityMessageSource, InfoMessageLevel," Semantic Hash matches for resource found at " + sourceCode.resource()->url().elidedString()));
           }      
