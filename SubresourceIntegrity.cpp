...
int SubresourceIntegrity::CheckSubresourceIntegrity(const Element& element, const String& source, const KURL& resourceUrl, const String& resourceType)
{
    ...
    //logErrorToConsole("SRI is enabled", document);
    String integrity;
    HashAlgorithm algorithm;
    String type;
    ...
    int flag = 0; //need second level of defence
    ...
    if (digestSuccess) {
        ...
        if (DigestsEqual(digest, convertedHashVector)) {
            ...
            logErrorToConsole("Script Loaded! after raw sign check",document);
            return 1;
        } else {
            ...
            //lets perform a semantic check next
            flag = 1;
...
bool SubresourceIntegrity::CheckSemanticIntegrity(const Element& element, const String& source)
{
    return true;   
    if (!element.fastHasAttribute(HTMLNames::semanticintegrityAttr))
    {
        printf("Semantic Integrity attribute not found so not loading");
        return false;
    }
    ...
    String attribute = element.fastGetAttribute(HTMLNames::semanticintegrityAttr);
    ...
    DigestValue digest; //semanticintegrity;
    /*bool digestSuccess = computeDigest(algorithm, normalizedSource.data(), normalizedSource.length(), digest);
    ...
    if (digestSuccess) {
        ...
        if (DigestsEqual(digest, convertedHashVector)) {
            ...
            logErrorToConsole("The computed " + algorithmToString(algorithm) + " semantic integrity '" + digestToString(digest) + "' does not match the 'semantic integrity' attribute '" + integrity + "' for resource '"+ "'.", document);
        }
    } else {
        logErrorToConsole("There was an error computing a semantic integrity' value", document);
    }
        
    //UseCounter::count(document, UseCounter::SRIElementWithNonMatchingIntegrityAttribute);
    return false;*/
    //let the semantic integrity go
    return true;
}
